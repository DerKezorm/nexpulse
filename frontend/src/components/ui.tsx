/** Grundbausteine im Look von Nexview und nexcrate. */
...
/** Ein Einstellungsbereich: Ueberschrift, Erklaerung, Inhalt, rechts optional ein Schalter oder Knopf. */
...
/**
 * Ein Schalter. ⚠️ Benannt ueber `aria-labelledby`: `<label for>` benennt keinen
 * Knopf mit `role="switch"` zuverlaessig.
 */
...
export function Banner({ tone = 'info', children }: { tone?: 'info' | 'bad' | 'ok'; children: ReactNode }) {
  const styles = {
    info: 'border-accent-500/30 bg-accent-500/8 text-mist-200',
    ok: 'border-accent-500/40 bg-accent-500/10 text-accent-400',
    bad: 'border-bad-500/40 bg-bad-500/10 text-bad-500',
  }[tone]
  const symbol: SymbolName = tone === 'bad' ? 'warn' : tone === 'ok' ? 'check' : 'info'
  return (
...
export function Badge({ tone = 'neutral', children }: { tone?: 'neutral' | 'accent' | 'bad' | 'up'; children: ReactNode }) {
  const styles = {
