"""Nachsehen, ob es eine neuere nexpulse-Version gibt.

nexpulse fragt dafuer hoechstens einmal am Tag die oeffentliche GitHub-API nach
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        antwort = await client.get(
            ...
            headers={
                ...
                "User-Agent": f"nexpulse/{__version__}",
            },
        )
...
async def status(*, enabled: bool = True, force: bool = False) -> UpdateStatus:
    ...
    frisch = _cached is not None and _cached.checked_at is not None and jetzt - _cached.checked_at < CHECK_INTERVAL
